...
#| code-fold: true
med_clean %>% 
  filter(species %in% c(groupers, diplodus, herbivores), !is.na(depth)) %>% 
  pivot_wider(names_from = species, values_from = sp.n, values_fill = 0, values_fn = sum) %>% 
  distinct(site, trans) %>% 
...
```
Note that these are **count** data and we used **biomass** data for the models.

Create coordinate datasets to run the spatial models
```{r}
grps_cords <- med_clean %>% filter(species %in% groupers, !is.na(depth)) %>% 
  pivot_wider(names_from = species, values_from = biomass, values_fill = 0, values_fn = sum) %>% 
  select(lon, lat)

dip_cords <- med_clean %>% filter(species %in% diplodus, !is.na(depth)) %>% 
  pivot_wider(names_from = species, values_from = biomass, values_fill = 0, values_fn = sum) %>% 
  select(lon, lat)

herb_cords <- med_clean %>% filter(species %in% herbivores, !is.na(depth)) %>% 
  pivot_wider(names_from = species, values_from = biomass, values_fill = 0, values_fn = sum) %>% 
  select(lon, lat)
```
