...
tibble(Group = c("Groupers", "Seabreams", "Herbivores"),
       Mean_R2 = c(cv.biomass.list$groupers %>% filter(model == "CRF") %>% summarise(mean(Rsquared)) %>% as.double(), 
                   cv.biomass.list$seabreams %>% filter(model == "CRF") %>% summarise(mean(Rsquared)) %>% as.double(), 
                   cv.biomass.list$herbivores %>% filter(model == "CRF") %>% summarise(mean(Rsquared)) %>% as.double()
))
```
